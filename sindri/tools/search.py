...
- search_code(query="class.*Handler", regex=true) - Regex search for Handler classes"""
    ...
    async def _text_search(
        ...
        try:
            ...
            return ToolResult(
                success=False, output="", error=f"Text search failed: {str(e)}"
            )
    ...
    async def _ripgrep_search(
        ...
        effective_max = (
            max_results * (1 + 2 * context_lines) if context_lines else max_results
        )
        ...
        if proc.returncode == 2:
            return ToolResult(
                success=False, output="", error=f"ripgrep error: {errors}"
            )
    ...
    async def _semantic_search(
        self, query: str, path: Path, file_types: Optional[list[str]], max_results: int
    ) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                success=False, output="", error=f"Semantic search failed: {str(e)}"
            )
...
- find_symbol(name="config", symbol_type="variable", file_types=["py"]) - Find config variable in Python"""
    ...
    async def execute(
        ...
        **kwargs,
        ...
        if not name.strip():
            return ToolResult(
                success=False, output="", error="Symbol name cannot be empty"
            )
        ...
        search_types = (
            ["function", "class", "variable"] if symbol_type == "any" else [symbol_type]
        )
