...
- rename_symbol(old_name="MAX_RETRIES", new_name="MAX_ATTEMPTS", file_types=["py"]) - Rename constant"""
    ...
    async def execute(
        ...
        **kwargs,
        ...
        if not self._is_valid_identifier(old_name):
            return ToolResult(
                success=False, output="", error=f"Invalid identifier: {old_name}"
            )
        if not self._is_valid_identifier(new_name):
            return ToolResult(
                success=False, output="", error=f"Invalid identifier: {new_name}"
            )
        ...
        if not search_path.exists():
            return ToolResult(
                success=False, output="", error=f"Path does not exist: {search_path}"
            )
        ...
        try:
            ...
            return ToolResult(
                success=False, output="", error=f"Failed to rename symbol: {str(e)}"
            )
    ...
    async def _find_files_with_symbol(
        self, search_path: Path, symbol: str, extensions: list[str]
    ) -> list[Path]:
    ...
    async def execute(
        ...
        **kwargs,
        ...
        if end_line < start_line:
            return ToolResult(
                success=False, output="", error="end_line must be >= start_line"
            )
        ...
        if not file_path.exists():
            return ToolResult(success=False, output="", error=f"File not found: {file}")
        ...
        try:
            ...
            original_indent = self._detect_indent(
                extracted_lines[0] if extracted_lines else ""
            )
            ...
            return ToolResult(
                success=False, output="", error=f"Failed to extract function: {str(e)}"
            )
    ...
    async def execute(
        ...
        **kwargs,
        ...
        if not variable or not re.match(r"^[a-zA-Z_][a-zA-Z0-9_]*$", variable):
            return ToolResult(
                success=False, output="", error=f"Invalid variable name: {variable}"
            )
        ...
        try:
            ...
            for line_idx, start, end in reversed(usages):
                ...
                new_lines[line_idx] = (
                    line_content[:start] + wrapped_value + line_content[end:]
                )
            ...
            return ToolResult(
                success=False, output="", error=f"Failed to inline variable: {str(e)}"
            )

    def _find_assignment(
        self, lines: list[str], variable: str, target_line: Optional[int], ext: str
...
- move_file(source="src/model.py", destination="src/models/user.py", dry_run=true) - Preview changes"""
    ...
    async def execute(
        ...
        **kwargs,
        ...
        if not source_path.exists():
            return ToolResult(
                success=False, output="", error=f"Source file not found: {source}"
            )
        ...
        try:
            ...
            if update_imports and source_module != dest_module:
                ...
                if import_updates["count"] > 0:
                    ...
                    output_lines.append(
                        f"{action} {import_updates['count']} import(s) in {len(import_updates['files'])} file(s)"
                    )
                    for file_info in import_updates["files"][:10]:  # Show first 10
                        output_lines.append(
                            f"  - {file_info['file']}: {file_info['count']} import(s)"
                        )
                    if len(import_updates["files"]) > 10:
                        output_lines.append(
                            f"  ... and {len(import_updates['files']) - 10} more files"
                        )
            ...
            if not dry_run:
                ...
                await self._update_internal_imports(
                    dest_path, source_path.parent, dest_path.parent
                )
            ...
            return ToolResult(
                success=True, output="\n".join(output_lines), metadata=results
            )

        except Exception as e:
            log.error("move_file_error", error=str(e))
            return ToolResult(
                success=False, output="", error=f"Failed to move file: {str(e)}"
            )
    ...
    async def _update_imports(
        ...
        for ext, lang in self.IMPORT_FILE_TYPES.items():
            for file_path in search_root.rglob(f"*{ext}"):
                ...
                if count > 0:
                    results["files"].append({"file": str(file_path), "count": count})
                    results["count"] += count
    ...
    async def _update_imports_in_file(
        ...
        elif file_lang in ["javascript", "typescript"] and source_lang in [
            "javascript",
            "typescript",
        ]:
    ...
    def _update_python_imports(
        ...
        pattern = re.compile(
            r"^(\s*import\s+)" + re.escape(old_module) + r"(\s*(?:#.*)?)$", re.MULTILINE
        )
        ...
        pattern = re.compile(
            r"^(\s*import\s+)" + re.escape(old_module) + r"(\s+as\s+\w+\s*(?:#.*)?)$",
            re.MULTILINE,
        )
        content, n = pattern.subn(rf"\g<1>{new_module}\g<2>", content)
        update_count += n
        ...
        pattern = re.compile(
            r"^(\s*from\s+)" + re.escape(old_module) + r"(\s+import\s+)", re.MULTILINE
        )
        content, n = pattern.subn(rf"\g<1>{new_module}\g<2>", content)
        update_count += n
        ...
        pattern = re.compile(
            r"^(\s*from\s+)" + re.escape(old_module) + r"\.(\S+\s+import\s+)",
            re.MULTILINE,
        )
        content, n = pattern.subn(rf"\g<1>{new_module}.\g<2>", content)
        update_count += n
    ...
    def _update_js_imports(
        self, content: str, source_path: Path, dest_path: Path, importing_file: Path
    ) -> Tuple[str, int]:
        ...
        if not old_rel.startswith("./") and not old_rel.startswith("../"):
            ...
            for pattern in patterns:
                ...
                pattern_with_dot = pattern.replace(
                    re.escape(old_rel), re.escape(old_rel_with_dot)
                )
    ...
    def _compute_js_import_path(self, target_path: Path, from_dir: Path) -> str:
        ...
        try:
            ...
            try:
                common = Path(from_dir.resolve()).relative_to(
    ...
    async def _update_internal_imports(
        self, file_path: Path, old_dir: Path, new_dir: Path
    ) -> None:
...
- batch_rename(pattern="*.js", output="{stem}.ts", dry_run=true) - Preview converting JS to TS"""
    ...
    async def execute(
        ...
        **kwargs,
        ...
        if not pattern or not pattern.strip():
            return ToolResult(success=False, output="", error="Pattern cannot be empty")
        if not output or not output.strip():
            return ToolResult(
                success=False, output="", error="Output pattern cannot be empty"
            )
        if max_files < 1:
            return ToolResult(
                success=False, output="", error="max_files must be at least 1"
            )
        ...
        try:
            ...
            if regex:
                ...
                matched_files = self._find_files_regex(
                    search_path, regex_pattern, recursive
                )
            else:
            ...
            for file_path in matched_files:
                try:
                    if regex:
                        new_name = self._apply_regex_output(
                            file_path, regex_pattern, output
                        )
                    else:
                    ...
                    if new_path.resolve() in seen_destinations:
                        conflicts.append(
                            f"Multiple files would be renamed to: {new_path}"
                        )
                    elif (
                        new_path.exists() and new_path.resolve() != file_path.resolve()
                    ):
                        ...
                        rename_plan.append(
                            {
                                ...
                                "dest_rel": (
                                    str(new_path.relative_to(search_path))
                                    if new_path.is_relative_to(search_path)
                                    else str(new_path)
                                ),
                            }
                        )
                ...
                if update_imports:
                    ...
                    if move_result.success:
                        ...
                        if move_result.metadata:
                            results["imports_updated"] += move_result.metadata.get(
                                "imports_updated", 0
                            )
                            if move_result.metadata.get("files_updated"):
                                results["files_with_import_updates"] += len(
                                    move_result.metadata["files_updated"]
                                )
                        results["renames"].append(
                            {
                                ...
                                "imports_updated": (
                                    move_result.metadata.get("imports_updated", 0)
                                    if move_result.metadata
                                    else 0
                                ),
                            }
                        ...
                        output_lines.append(
                            f"  {plan['source_rel']} → {plan['dest_rel']}"
                        )
                    else:
                        # Report error but continue with other files
                        output_lines.append(
                            f"  ✗ {plan['source_rel']}: {move_result.error}"
                        )
                else:
            ...
            return ToolResult(
                success=True, output="\n".join(summary_lines), metadata=results
            )

        except Exception as e:
            log.error("batch_rename_error", error=str(e))
            return ToolResult(
                success=False, output="", error=f"Batch rename failed: {str(e)}"
            )

    def _find_files_glob(
        self, search_path: Path, pattern: str, recursive: bool
    ) -> list[Path]:
        ...
        if recursive:
            ...
            for file_path in search_path.rglob(pattern):
                if file_path.is_file() and not any(
                    part in self.SKIP_DIRS for part in file_path.parts
                ):
                    files.append(file_path)
        else:
    ...
    def _find_files_regex(
        self, search_path: Path, regex_pattern: re.Pattern, recursive: bool
    ) -> list[Path]:
    ...
    def _apply_regex_output(
        self, file_path: Path, regex_pattern: re.Pattern, output: str
    ) -> str:
...
- split_file(file="src/all.py", strategy="classes", dry_run=true) - Preview split without applying"""
    ...
    parameters = {
        "type": "object",
        "properties": {
            "file": {"type": "string", "description": "Path to the file to split"},
            "strategy": {
                "type": "string",
    ...
    async def execute(
        ...
        **kwargs,
        ...
        if not source_path.exists():
            return ToolResult(success=False, output="", error=f"File not found: {file}")
        if not source_path.is_file():
            return ToolResult(success=False, output="", error=f"Not a file: {file}")
        ...
        try:
            ...
            if strategy == "classes":
                ...
                splits = self._split_by_markers(
                    content, marker or "# --- split: {filename} ---"
                )
            else:  # lines
            ...
            for split in splits:
                ...
                if dry_run:
                    preview_content = (
                        split["content"][:200] + "..."
                        if len(split["content"]) > 200
                        else split["content"]
                    )
                    output_lines.append(f"  Would create: {dest_file}")
                    output_lines.append(
                        f"    Content preview ({split.get('lines', 'N/A')} lines):"
                    )
            ...
            if update_imports and not dry_run:
                ...
                if import_results["count"] > 0:
                    output_lines.append(
                        f"  Updated {import_results['count']} import(s) in {len(import_results['files'])} file(s)"
                    )

            # Keep original file with re-exports for backward compatibility
            if keep_original and not dry_run and lang == "python":
                reexport_content = self._generate_reexport_content(
                    splits, output_path, source_path
                )
                async with aiofiles.open(source_path, "w", encoding="utf-8") as f:
                    await f.write(reexport_content)
                output_lines.append(
                    "  Updated original file with re-exports for backward compatibility"
                )

            # Build summary
            action = "Would split" if dry_run else "Split"
            summary = (
                f"{action} {file} into {len(splits)} files using '{strategy}' strategy"
            )
            ...
            return ToolResult(
                success=False, output="", error=f"Failed to split file: {str(e)}"
            )

    def _split_by_classes(
        self, content: str, source_path: Path, lang: str
    ) -> list[dict]:
    ...
    def _split_by_lines(
        self, content: str, split_lines: list[int], source_path: Path
    ) -> list[dict]:
    ...
    def _generate_init_content(self, splits: list[dict], original_name: str) -> str:
        ...
        lines = [f'"""Package created by splitting {original_name}."""', ""]
        ...
        # Generate __all__
        all_items = [
            split.get("name", Path(split["filename"]).stem) for split in splits
        ]
    ...
    def _generate_reexport_content(
        self, splits: list[dict], output_dir: Path, source_path: Path
    ) -> str:
        """Generate content for original file with re-exports."""
        lines = ['"""Re-exports from split modules for backward compatibility."""', ""]
    ...
    async def _update_imports_for_split(
        self, source_path: Path, splits: list[dict], output_dir: Path, search_root: Path
    ) -> dict:
        ...
        try:
            old_module = ".".join(
                source_path.relative_to(search_root).with_suffix("").parts
            )
        except ValueError:
...
- merge_files(files=["foo.ts", "bar.ts"], destination="index.ts") - Merge TypeScript"""
    ...
    async def execute(
        ...
        **kwargs,
        ...
        if (
            dest_path.resolve() in [f.resolve() for f in source_files]
            and not delete_sources
        ):
            return ToolResult(
                success=False,
                output="",
        ...
        try:
            ...
            for source_file in source_files:
                ...
                file_contents.append(
                    {"path": source_file, "content": content, "name": source_file.stem}
                )
            ...
            if dry_run:
                output_lines.append(
                    f"Would merge {len(source_files)} files into {destination}"
                )
                output_lines.append("")
                ...
                output_lines.append("")
                output_lines.append(
                    f"Merged content preview ({len(merged_content)} chars, {merged_content.count(chr(10))+1} lines):"
                )
                preview = (
                    merged_content[:500] + "..."
                ...
                output_lines.append(
                    f"Merged {len(source_files)} files into {destination}"
                )
            ...
            return ToolResult(
                success=False, output="", error=f"Failed to merge files: {str(e)}"
            )

    def _merge_files(
        self, file_contents: list[dict], lang: str, add_section_comments: bool
    ) -> Tuple[str, list[str]]:
    ...
    def _merge_python_files(
        self, file_contents: list[dict], add_section_comments: bool
    ) -> Tuple[str, list[str]]:
        ...
        for fc in file_contents:
            ...
            for node in ast.iter_child_nodes(tree):
                if isinstance(node, ast.Import):
                    ...
                    non_import_start = max(
                        non_import_start, node.end_lineno or node.lineno
                    )
    ...
    async def _update_imports_for_merge(
        ...
        for source_file in source_files:
            try:
                module = ".".join(
                    source_file.relative_to(search_root).with_suffix("").parts
                )
            except ValueError:
        ...
        try:
            dest_module = ".".join(
                dest_path.relative_to(search_root).with_suffix("").parts
            )
        except ValueError:
    ...
    async def _update_imports_in_file_for_merge(
        ...
        for old_module in source_modules:
            ...
            pattern = re.compile(
                rf"^(\s*import\s+){re.escape(old_module)}(\s*(?:#.*)?)$", re.MULTILINE
            )
